...
import {GET_PRODUCT, GET_PRODUCTS, GET_SEARCHED_PRODUCTS, IS_LOADING, SET_ERROR} from "./constants";
...
export const getProductByText = (text, message) => async dispatch => {
    try {
        const response = await axios.get(`${API_URL}/getproductbytext/${text}`)
        if (!response?.data?.productFound) {
            message.warning(response.data.message)
            dispatch({type: GET_SEARCHED_PRODUCTS, payload: response.data.products})
        } else {
        ...
        dispatch({type: SET_ERROR, payload: {message: "An error had occurred"}});
    }
}
